...
zcat /scratch/${WORKING_DIRECTORY}/${SNAPSHOT_FILE} | parallel --pipe --block 100M --jobs 6 --files --tmpdir /scratch/${WORKING_DIRECTORY}/${SNAPSHOT_DIRECTORY} --recend '}\n' "jq -c 'select(.year >= 2008 and .year <= 2023) | {doi, genre, has_repository_copy, is_paratext, is_oa, journal_is_in_doaj, journal_is_oa, journal_issn_l, journal_issns, journal_name, oa_locations, oa_locations_embargoed, oa_status, publisher, published_date, doi_updated: .updated, year}'"
